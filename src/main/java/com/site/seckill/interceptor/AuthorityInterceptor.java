package com.site.seckill.interceptor;

import com.site.seckill.annotations.AccessLimit;
import com.site.seckill.entity.User;
import com.site.seckill.redis.AccessKey;
...
//        System.out.println("拦截器执行了");
        //请求controller中的方法名
        HandlerMethod handlerMethod = (HandlerMethod) handler;//反射
        //获取方法名
        String methodName = handlerMethod.getMethod().getName();
        //获取类名
        String className = handlerMethod.getBean().getClass().getSimpleName();
        StringBuffer requestParamBuffer = new StringBuffer();
        //request.getParameterMap()返回的是一个Map类型的值，该返回值记录着前端（如jsp页面）所提交请求中的请求参数和请求参数值的映射关系
        ...
        if(count  == null) {
            redisService.set(ak, key, 1, seconds);//seconds为过期时间，value为请求次数
        }else if(count < maxCount) {
            redisService.incr(ak, key);//原子递增访问次数
        }else {
